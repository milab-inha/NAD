...
class QKVFlashAttention(nn.Module):
    def __init__(
        ...
        num_heads,
        batch_first=True,
        attention_dropout=0.0,   # v2.7 flash_attn_func에서는 사실상 사용 X
        causal=False,
        device=None,
        dtype=None,
        **kwargs,
        ...
        assert batch_first, "QKVFlashAttention currently assumes batch_first=True"
        factory_kwargs = {"device": device, "dtype": dtype}
        ...
        # v2 계열에서는 head_dim이 256까지 지원됨
        # (예전처럼 16/32/64로만 제한할 필요는 없음)
        assert self.head_dim <= 256, "flash-attn v2 supports head_dim up to 256"

        # v1: self.inner_attn = FlashAttention(...)
        # v2: 함수 핸들만 들고 있다가 forward에서 호출
    ...
    def forward(self, qkv, attn_mask=None, key_padding_mask=None, need_weights=False):
        # flash_attn_func는 padding mask / attn_mask / attn_weights 반환을 직접 지원 안 함
        # (필요하면 여기서 fallback을 타거나 별도 구현 필요)
        assert attn_mask is None, "flash_attn_func does not support attn_mask"
        assert key_padding_mask is None, "flash_attn_func does not support key_padding_mask"
        assert not need_weights, "flash_attn_func does not return attention weights"

        if not hasattr(self, "_debug_once"):
            print("[flash-attn] using flash kernel")
            self._debug_once = True

        # qkv: (B, 3 * H * D, S) -> (B, S, 3, H, D)
        qkv = self.rearrange(
            qkv, "b (three h d) s -> b s three h d", three=3, h=self.num_heads
        )
        ...
        # flash-attn은 dtype/contiguous에 민감함
        # (보통 모델 전체를 fp16/bf16으로 돌리면 자연스럽게 맞음)
        q = q.contiguous()
        k = k.contiguous()
        v = v.contiguous()

        # flash_attn_func:
        #   out = flash_attn_func(q, k, v, softmax_scale=None, causal=self.causal, ...)
        # v2.7 hopper 인터페이스 기준으로 dropout_p 인자는 없음
        # flash-attn's custom autograd Function does not implement setup_context,
        # which breaks under functorch transforms (vjp/jacrev used by DAPS).
        # Fall back to a vanilla attention computation in that case.
        try:
            ...
            if "setup_context" not in str(e):
                raise
            if not hasattr(self, "_fallback_once"):
                print("[flash-attn] falling back to legacy attention (functorch transforms detected)")
                self._fallback_once = True
            scale = 1.0 / math.sqrt(self.head_dim)
            q_bhsd = q.transpose(1, 2)  # (B, H, S, D)
            k_bhsd = k.transpose(1, 2)
            v_bhsd = v.transpose(1, 2)
            attn = th.softmax(th.matmul(q_bhsd, k_bhsd.transpose(-1, -2)) * scale, dim=-1)
            out = th.matmul(attn, v_bhsd).transpose(1, 2)
        # out: (B, S, H, D) -> (B, H * D, S)
        out = self.rearrange(out, "b s h d -> b (h d) s")
        return out
